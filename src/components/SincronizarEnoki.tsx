...
export function SincronizarEnoki() {
  const { estado, sincronizarEnoki } = useDre()
  ...
  return (
    <Card className="animate-rise">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Botao onClick={rodar} disabled={rodando}>
            ...
            <div className="mt-3 grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                {sync.residuos.length ? (
                  <ul className="space-y-1 text-xs text-muted">
                    {sync.residuos.map((r) => (
                      <li key={`${r.centroCusto}|${r.fluxo}`} className="flex justify-between gap-3">
                        <span>
                          {r.centroCusto} <span className="text-faint">({r.fluxo})</span>
                        </span>
                        <span className="whitespace-nowrap tabular-nums">
